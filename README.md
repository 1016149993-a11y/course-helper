...
- **自动下一章节（连播）**：默认关闭，需在面板手动勾选；开启后每 3 秒自动推进——未播完的视频自动续播、本页看完或无视频页面自动进入下一页、弹窗自动关闭；遇到题目页自动停下等待手动作答

> 本脚本**不会**模拟播放、挂机刷时长、伪造观看记录或自动刷题。自动下一章节仅为连播辅助，播放本身仍真实进行，高倍速 + 连播可能被平台判定为异常，风险自担。
