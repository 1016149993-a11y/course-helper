...
- **跨平台题目页检测与自动答题**：统一识别学习通 / 优学院 / U校园 的题目页，
  并支持自动选择答案与自动提交
  - 优学院：`.question-setting-panel` / `.question-area` 等原有特征继续兼容
  - 学习通：新增 `.questionLi` / `.Zy_TItle` / `.chaoxingquiz` / `.jobQuize` 等常见题目页特征
  - U校园/WeLearn：新增 `.question-box` / `.test-container` / `.quiz-wrapper` 等特征
  - 通用兜底：URL 路径含 `quiz` / `exam` / `test` / `work` 且页面存在单选/多选/填空元素时
    也视为题目页
- **面板新增“自动答题”与“自动提交”开关**：两者默认关闭，可独立控制；
  开启后脚本会自动选择占位答案并点击提交按钮
- **答案源接口（AnswerSource）**：抽象出可插拔的答案源
  - `firstOptionSource()`：默认源，未命中时选首个选项/填占位符
  - `questionBankSource(bank)`：题库源，按题目文本精确/模糊匹配
...
- **已作答题目页自动继续**：题目页检测会判断当前是否已提交/已完成，
  已完成时允许继续推进，避免用户手动提交后连播仍卡住
- **答题提交后自动恢复连播**：通过事件委托监听提交按钮点击，
  点击后 1.5 秒自动清除题目页暂停状态，继续自动下一章节
- **暂停状态持久化 + 防残留**：题目页暂停状态写入 localStorage，
  刷新后仍保持暂停；但 10 分钟后自动过期，切页后也会自动清除，防止异常卡死
- **自动答题策略**：优先查答案源；答案源未命中时兜底选首个选项，保证作答完成率
...
- **学习通卡内翻页（"下一节"按钮）**：学习通一个章节含多张卡（课件卡 + 视频卡），
  此前无视频页直接跳下一章会跳过同章节的视频卡。新增 `chaoxingAdvance()`：
  点击学习通"下一节"按钮（`#prevNextFocusNext` / `.prev_next.next`，调用 `PCount.next`）
  翻到下一张卡，保留在同章节内；当前卡有未播完视频时不翻卡，让视频先播
- 优先级：优学院推进 → 学习通"下一节" → 通用章节列表 / 自动播放
...
- **学习通自动播放被浏览器拦截**：学习通视频在 iframe 中，无真实用户手势，
  Chrome 拒绝非静音自动播放。`autoPlayVideos` 现在先尝试有声播放，被拦截后
  自动降级为**静音播放**（浏览器允许静音自动播放）；用户需要声音时手动取消静音
